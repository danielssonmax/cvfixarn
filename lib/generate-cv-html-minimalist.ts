// Generate static HTML from CV data for Minimalist template
export function generateCVHtmlMinimalist(data: any, sectionOrder: string[], sections: any[]): string {
  ...
  // Minimalist template - centered header, single column
  let html = '<div class="cv-container-minimalist">'
  
  // Header - centered name and title
  const fullName = `${safePersonalInfo.firstName} ${safePersonalInfo.lastName}`.trim() || 'Curriculum Vitae'
  
  html += `<div class="cv-header-minimalist">`
  
  // Profile photo (centered above name)
  if (isValidImageSrc(safePersonalInfo.photo)) {
    html += `<div class="cv-photo-container-minimalist">`
    html += `<img src="${safePersonalInfo.photo}" alt="Profile" class="cv-profile-image-minimalist" />`
    html += `</div>`
  }
  
  html += `<h1 class="cv-name-minimalist">${fullName}</h1>`
  if (safePersonalInfo.title) html += `<p class="cv-title-minimalist">${safePersonalInfo.title}</p>`
  
  // Contact info in one line
  const contactItems = []
  if (safePersonalInfo.email) contactItems.push(safePersonalInfo.email)
  if (safePersonalInfo.phone) contactItems.push(safePersonalInfo.phone)
  const locationParts = [safePersonalInfo.address, safePersonalInfo.postalCode, safePersonalInfo.location].filter(v => v && v.trim() !== '')
  if (locationParts.length > 0) contactItems.push(locationParts.join(', '))
  
  if (contactItems.length > 0) {
    html += `<p class="cv-contact-minimalist">${contactItems.join(' • ')}</p>`
  }
  ...
  if (safePersonalInfo.optionalFields && typeof safePersonalInfo.optionalFields === 'object') {
    ...
    if (optionalItems.length > 0) {
      html += `<p class="cv-contact-minimalist">${optionalItems.join(' • ')}</p>`
    }
  }
  ...
  sectionOrder.forEach(sectionId => {
    ...
    switch (sectionId) {
      case 'profile':
        ...
        html += `<div class="cv-section-minimalist">`
        html += `<h2 class="cv-section-title-minimalist">${sectionToUse.title || 'PROFIL'}</h2>`
      ...
      case 'experience':
        ...
        html += `<div class="cv-section-minimalist">`
        html += `<h2 class="cv-section-title-minimalist">${sectionToUse.title || 'ARBETSLIVSERFARENHET'}</h2>`
        data.workExperience.forEach((exp: any) => {
          ...
          html += `<div class="cv-item-minimalist" data-keep>`
          html += `<div class="cv-item-header-minimalist">`
          if (expTitle) html += `<h3 class="cv-item-title-minimalist">${expTitle}</h3>`
          if (dates) html += `<span class="cv-item-date-minimalist">${dates}</span>`
          html += `</div>`
          if (expCompany || expLocation) {
            const companyLocation = [expCompany, expLocation].filter(v => v && v.trim() !== '').join(', ')
            html += `<p class="cv-item-subtitle-minimalist">${companyLocation}</p>`
          }
      ...
      case 'education':
        ...
        html += `<div class="cv-section-minimalist">`
        html += `<h2 class="cv-section-title-minimalist">${sectionToUse.title || 'UTBILDNING'}</h2>`
        data.education.forEach((edu: any) => {
          ...
          html += `<div class="cv-item-minimalist" data-keep>`
          html += `<div class="cv-item-header-minimalist">`
          if (eduDegree) html += `<h3 class="cv-item-title-minimalist">${eduDegree}</h3>`
          if (dates) html += `<span class="cv-item-date-minimalist">${dates}</span>`
          html += `</div>`
          if (eduSchool || eduLocation) {
            const schoolLocation = [eduSchool, eduLocation].filter(v => v && v.trim() !== '').join(', ')
            html += `<p class="cv-item-subtitle-minimalist">${schoolLocation}</p>`
          }
      ...
      case 'skills':
        ...
        html += `<div class="cv-section-minimalist">`
        html += `<h2 class="cv-section-title-minimalist">${sectionToUse.title || 'FÄRDIGHETER'}</h2>`
        html += `<div class="cv-skills-minimalist">`
        data.skills.forEach((skill: any) => {
          if (!hasContent(skill)) return
          const skillName = safeText(skill.name)
          const skillLevel = safeText(skill.level)
          if (skillName) html += `<span class="cv-skill-item-minimalist">${skillName}${skillLevel ? ` • ${skillLevel}` : ''}</span>`
        })
        html += `</div></div>`
        break
      ...
      case 'languages':
        ...
        html += `<div class="cv-section-minimalist">`
        html += `<h2 class="cv-section-title-minimalist">${sectionToUse.title || 'SPRÅK'}</h2>`
        html += `<div class="cv-skills-minimalist">`
        data.languages.forEach((lang: any) => {
          ...
          if (langName) html += `<span class="cv-skill-item-minimalist">${langName}${langLevel ? ` • ${langLevel}` : ''}</span>`
        })
        html += `</div></div>`
        break
      ...
      case 'courses':
        ...
        html += `<div class="cv-section-minimalist">`
        html += `<h2 class="cv-section-title-minimalist">${sectionToUse.title || 'KURSER'}</h2>`
        data.sections.courses.forEach((course: any) => {
          ...
          html += `<div class="cv-item-minimalist" data-keep>`
          html += `<div class="cv-item-header-minimalist">`
          if (courseName) html += `<h3 class="cv-item-title-minimalist">${courseName}</h3>`
          if (courseYear) html += `<span class="cv-item-date-minimalist">${courseYear}</span>`
          html += `</div>`
          if (courseOrganization) html += `<p class="cv-item-subtitle-minimalist">${courseOrganization}</p>`
      ...
      case 'certificates':
        ...
        html += `<div class="cv-section-minimalist">`
        html += `<h2 class="cv-section-title-minimalist">${sectionToUse.title || 'CERTIFIKAT'}</h2>`
        data.sections.certificates.forEach((cert: any) => {
          ...
          html += `<div class="cv-item-minimalist" data-keep>`
          html += `<div class="cv-item-header-minimalist">`
          if (certName) html += `<h3 class="cv-item-title-minimalist">${certName}</h3>`
          if (certYear) html += `<span class="cv-item-date-minimalist">${certYear}</span>`
          html += `</div>`
          if (certIssuer) html += `<p class="cv-item-subtitle-minimalist">${certIssuer}</p>`
      ...
      case 'achievements':
        ...
        html += `<div class="cv-section-minimalist">`
        html += `<h2 class="cv-section-title-minimalist">${sectionToUse.title || 'PRESTATIONER'}</h2>`
        data.sections.achievements.forEach((achievement: any) => {
          ...
          html += `<div class="cv-item-minimalist" data-keep>`
          if (achievementTitle) html += `<h3 class="cv-item-title-minimalist">${achievementTitle}</h3>`
      ...
      case 'internship':
        ...
        html += `<div class="cv-section-minimalist">`
        html += `<h2 class="cv-section-title-minimalist">${sectionToUse.title || 'PRAKTIK'}</h2>`
        data.sections.internship.forEach((intern: any) => {
          ...
          html += `<div class="cv-item-minimalist" data-keep>`
          html += `<div class="cv-item-header-minimalist">`
          if (internTitle) html += `<h3 class="cv-item-title-minimalist">${internTitle}</h3>`
          if (dates) html += `<span class="cv-item-date-minimalist">${dates}</span>`
          html += `</div>`
      ...
      case 'traits':
        ...
        html += `<div class="cv-section-minimalist">`
        html += `<h2 class="cv-section-title-minimalist">${sectionToUse.title || 'EGENSKAPER'}</h2>`
        html += `<div class="cv-skills-minimalist">`
        validTraits.forEach((trait: any) => {
          html += `<span class="cv-skill-item-minimalist">${safeText(trait.name)}</span>`
        })
        html += `</div></div>`
        break
      ...
      case 'hobbies':
        ...
        html += `<div class="cv-section-minimalist">`
        html += `<h2 class="cv-section-title-minimalist">${sectionToUse.title || 'FRITIDSAKTIVITETER'}</h2>`
      ...
      case 'awards':
        ...
        html += `<div class="cv-section-minimalist">`
        html += `<h2 class="cv-section-title-minimalist">${sectionToUse.title || 'UTMÄRKELSER'}</h2>`
        data.sections.awards.forEach((award: any) => {
          ...
          html += `<div class="cv-item-minimalist" data-keep>`
          html += `<div class="cv-item-header-minimalist">`
          if (awardTitle) html += `<h3 class="cv-item-title-minimalist">${awardTitle}</h3>`
          if (awardDate) html += `<span class="cv-item-date-minimalist">${awardDate}</span>`
          html += `</div>`
          if (awardIssuer) html += `<p class="cv-item-subtitle-minimalist">${awardIssuer}</p>`
      ...
      case 'volunteering':
        ...
        html += `<div class="cv-section-minimalist">`
        html += `<h2 class="cv-section-title-minimalist">${sectionToUse.title || 'VOLONTÄRARBETE'}</h2>`
        data.sections.volunteering.forEach((vol: any) => {
          ...
          html += `<div class="cv-item-minimalist" data-keep>`
          html += `<div class="cv-item-header-minimalist">`
          if (volTitle) html += `<h3 class="cv-item-title-minimalist">${volTitle}</h3>`
          if (volDates) html += `<span class="cv-item-date-minimalist">${volDates}</span>`
          html += `</div>`
          if (volOrg || volLocation) {
            const orgLocation = [volOrg, volLocation].filter(v => v && v.trim() !== '').join(', ')
            html += `<p class="cv-item-subtitle-minimalist">${orgLocation}</p>`
          }
      ...
      case 'licenses':
        ...
        html += `<div class="cv-section-minimalist">`
        html += `<h2 class="cv-section-title-minimalist">${sectionToUse.title || 'LICENSER'}</h2>`
        data.sections.licenses.forEach((license: any) => {
          ...
          html += `<div class="cv-item-minimalist" data-keep>`
          html += `<div class="cv-item-header-minimalist">`
          if (licenseName) html += `<h3 class="cv-item-title-minimalist">${licenseName}</h3>`
          if (licenseDate) html += `<span class="cv-item-date-minimalist">${licenseDate}</span>`
          html += `</div>`
          if (licenseIssuer) html += `<p class="cv-item-subtitle-minimalist">${licenseIssuer}</p>`
          html += `</div>`
        })
        html += `</div>`
        break
      ...
      case 'custom':
        ...
        html += `<div class="cv-section-minimalist">`
        html += `<h2 class="cv-section-title-minimalist">${sectionToUse.title || 'ÖVRIGT'}</h2>`
        data.sections.custom.forEach((entry: any) => {
          ...
          html += `<div class="cv-item-minimalist" data-keep>`
          if (entryTitle) html += `<h3 class="cv-item-title-minimalist">${entryTitle}</h3>`
      ...
      case 'references':
        html += `<div class="cv-section-minimalist">`
        html += `<h2 class="cv-section-title-minimalist">${sectionToUse.title || 'REFERENSER'}</h2>`
        ...
        if (!hasActualReferences) {
          ...
          data.sections.references.forEach((ref: any) => {
            ...
            html += `<div class="cv-item-minimalist" data-keep>`
            if (refName) html += `<h3 class="cv-item-title-minimalist">${refName}</h3>`
            
            if (refTitle || refCompany) {
              const titleCompanyParts = [refTitle, refCompany].filter(v => v && v.trim() !== '')
              html += `<p class="cv-item-subtitle-minimalist">${titleCompanyParts.join(', ')}</p>`
            }
