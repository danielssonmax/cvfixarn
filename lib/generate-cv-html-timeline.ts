// Generate static HTML from CV data for Timeline template
export function generateCVHtmlTimeline(data: any, sectionOrder: string[], sections: any[]): string {
  ...
  const dateRange = (startDate?: string, startYear?: string, endDate?: string, endYear?: string, current?: boolean): string => {
    ...
    const end = current ? 'Nu' : (endYear || endDate || '')
    if (!start && !end) return ''
    if (!end) return start
    return `${start} - ${end}`
  }
  ...
  // Timeline template - vertical timeline for experience and education
  let html = '<div class="cv-container-timeline">'
  
  const fullName = `${safePersonalInfo.firstName} ${safePersonalInfo.lastName}`.trim() || 'Curriculum Vitae'
  
  // Header
  html += `<div class="cv-header-timeline">`
  html += `<div class="cv-header-content-timeline">`
  
  // Photo on the left (smaller)
  if (isValidImageSrc(safePersonalInfo.photo)) {
    html += `<div class="cv-photo-timeline">`
    html += `<img src="${safePersonalInfo.photo}" alt="Profile" class="cv-profile-image-timeline" />`
    html += `</div>`
  }
  
  // Name and info on the right
  html += `<div class="cv-header-text-timeline">`
  html += `<h1 class="cv-name-timeline">${fullName}</h1>`
  if (safePersonalInfo.title) html += `<p class="cv-title-timeline">${safePersonalInfo.title}</p>`
  ...
  if (contactItems.length > 0) {
    html += `<p class="cv-contact-timeline">${contactItems.join(' • ')}</p>`
  }
  ...
  if (safePersonalInfo.optionalFields && typeof safePersonalInfo.optionalFields === 'object') {
    ...
    if (optionalItems.length > 0) {
      html += `<p class="cv-contact-timeline">${optionalItems.join(' • ')}</p>`
    }
  }
  
  html += `</div>` // End header-text
  ...
  sectionOrder.forEach(sectionId => {
    ...
    switch (sectionId) {
      case 'profile':
        ...
        html += `<h2 class="cv-section-title-timeline">${sectionToUse.title || 'PROFIL'}</h2>`
      ...
      case 'experience':
        ...
        html += `<div class="cv-section-timeline">`
        html += `<h2 class="cv-section-title-timeline">${sectionToUse.title || 'ARBETSLIVSERFARENHET'}</h2>`
        ...
        data.workExperience.forEach((exp: any, index: number) => {
          ...
          if (dates) html += `<span class="cv-timeline-date">${dates}</span>`
          if (expTitle) html += `<h3 class="cv-timeline-title">${expTitle}</h3>`
          if (expCompany || expLocation) {
            const companyLocation = [expCompany, expLocation].filter(v => v && v.trim() !== '').join(' • ')
            html += `<p class="cv-timeline-subtitle">${companyLocation}</p>`
          }
      ...
      case 'education':
        ...
        html += `<div class="cv-section-timeline">`
        html += `<h2 class="cv-section-title-timeline">${sectionToUse.title || 'UTBILDNING'}</h2>`
        html += `<div class="cv-timeline">`
        data.education.forEach((edu: any) => {
          ...
          html += `<div class="cv-timeline-content">`
          if (dates) html += `<span class="cv-timeline-date">${dates}</span>`
          if (eduDegree) html += `<h3 class="cv-timeline-title">${eduDegree}</h3>`
          if (eduSchool || eduLocation) {
            const schoolLocation = [eduSchool, eduLocation].filter(v => v && v.trim() !== '').join(' • ')
            html += `<p class="cv-timeline-subtitle">${schoolLocation}</p>`
          }
      ...
      case 'skills':
        ...
        html += `<div class="cv-section-timeline">`
        html += `<h2 class="cv-section-title-timeline">${sectionToUse.title || 'FÄRDIGHETER'}</h2>`
        html += `<div class="cv-skills-timeline">`
        data.skills.forEach((skill: any) => {
          if (!hasContent(skill)) return
          const skillName = safeText(skill.name)
          const skillLevel = safeText(skill.level)
          if (skillName) html += `<span class="cv-skill-item-timeline">${skillName}${skillLevel ? ` • ${skillLevel}` : ''}</span>`
        })
        html += `</div></div>`
        break
      ...
      case 'languages':
        ...
        html += `<div class="cv-section-timeline">`
        html += `<h2 class="cv-section-title-timeline">${sectionToUse.title || 'SPRÅK'}</h2>`
        html += `<div class="cv-skills-timeline">`
        data.languages.forEach((lang: any) => {
          ...
          if (langName) html += `<span class="cv-skill-item-timeline">${langName}${langLevel ? ` • ${langLevel}` : ''}</span>`
        })
        html += `</div></div>`
        break
      ...
      case 'courses':
        ...
        html += `<div class="cv-section-timeline">`
        html += `<h2 class="cv-section-title-timeline">${sectionToUse.title || 'KURSER'}</h2>`
        html += `<div class="cv-timeline">`
        data.sections.courses.forEach((course: any) => {
          ...
          html += `<div class="cv-timeline-marker"></div>`
          html += `<div class="cv-timeline-content">`
          if (courseYear) html += `<span class="cv-timeline-date">${courseYear}</span>`
          if (courseName) html += `<h3 class="cv-timeline-title">${courseName}</h3>`
          if (courseOrganization) html += `<p class="cv-timeline-subtitle">${courseOrganization}</p>`
      ...
      case 'certificates':
        ...
        html += `<div class="cv-section-timeline">`
        html += `<h2 class="cv-section-title-timeline">${sectionToUse.title || 'CERTIFIKAT'}</h2>`
        html += `<div class="cv-timeline">`
        data.sections.certificates.forEach((cert: any) => {
          ...
          html += `<div class="cv-timeline-marker"></div>`
          html += `<div class="cv-timeline-content">`
          if (certYear) html += `<span class="cv-timeline-date">${certYear}</span>`
          if (certName) html += `<h3 class="cv-timeline-title">${certName}</h3>`
          if (certIssuer) html += `<p class="cv-timeline-subtitle">${certIssuer}</p>`
      ...
      case 'achievements':
        ...
        html += `<div class="cv-section-timeline">`
        html += `<h2 class="cv-section-title-timeline">${sectionToUse.title || 'PRESTATIONER'}</h2>`
        data.sections.achievements.forEach((achievement: any) => {
          ...
          if (achievementTitle) html += `<h3 class="cv-item-title-timeline">${achievementTitle}</h3>`
      ...
      case 'internship':
        ...
        html += `<div class="cv-section-timeline">`
        html += `<h2 class="cv-section-title-timeline">${sectionToUse.title || 'PRAKTIK'}</h2>`
        html += `<div class="cv-timeline">`
        data.sections.internship.forEach((intern: any) => {
          ...
          html += `<div class="cv-timeline-content">`
          if (dates) html += `<span class="cv-timeline-date">${dates}</span>`
          if (internTitle) html += `<h3 class="cv-timeline-title">${internTitle}</h3>`
      ...
      case 'traits':
        ...
        html += `<div class="cv-section-timeline">`
        html += `<h2 class="cv-section-title-timeline">${sectionToUse.title || 'EGENSKAPER'}</h2>`
        html += `<div class="cv-skills-timeline">`
        validTraits.forEach((trait: any) => {
          html += `<span class="cv-skill-item-timeline">${safeText(trait.name)}</span>`
        })
        html += `</div></div>`
        break
      ...
      case 'hobbies':
        ...
        html += `<div class="cv-section-timeline">`
        html += `<h2 class="cv-section-title-timeline">${sectionToUse.title || 'FRITIDSAKTIVITETER'}</h2>`
      ...
      case 'references':
        ...
        if (!hasActualReferences) {
          ...
          data.sections.references.forEach((ref: any) => {
            ...
            html += `<div class="cv-item-timeline" data-keep>`
            if (refName) html += `<h3 class="cv-item-title-timeline">${refName}</h3>`
            
            if (refTitle || refCompany) {
              const titleCompanyParts = [refTitle, refCompany].filter(v => v && v.trim() !== '')
              html += `<p class="cv-timeline-subtitle">${titleCompanyParts.join(' • ')}</p>`
            }
